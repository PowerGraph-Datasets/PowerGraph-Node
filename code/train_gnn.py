...
class TrainModel(object):
    ...
    def _train_batch(self, data, labels):
        ...
        if self.graph_classification:
            ...
            loss = self.__loss__(logits, labels)
        elif self.node_opf_regression or self.node_pf_regression:
            mask = data.mask
            if mask is None:
    ...
    def test(self):
        ...
        if self.node_pf_regression or self.node_opf_regression:
            ...
            return test_loss, test_r2score, denpreds, dentargets
...
def train_gnn(args, args_group):
    ...
    if eval(args.graph_regression):
        ...
        target_values = targets.cpu().numpy() if isinstance(targets, torch.Tensor) else np.array(targets)
        if args.datatype == 'nodeopf':
            df_data = pd.DataFrame({
                  'V target': target_values[:, 0],
                  'T target': target_values[:, 1],
                 'Pg target': target_values[:, 2],
                 'Qg target': target_values[:, 3],
                 'V pred': predicted_values[:, 0],
                 'T pred': predicted_values[:, 1],
                'Pg pred': predicted_values[:, 2],
                'Qg pred': predicted_values[:, 3],
            })
        else:
            df_data = pd.DataFrame({
