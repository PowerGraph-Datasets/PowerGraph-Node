...
def get_data_args(dataset, args):
     # make sure it is a one class problem
    if args.datatype.lower() == "binary" or args.datatype.lower() == "multiclass":
        ...
        assert dataset.data.y.ndim == 1
    if args.datatype.lower() == "regression":
        args.num_classes = 1
        assert dataset.data.y.ndim == 1
    if args.datatype.lower() == "node" or args.datatype.lower() == "nodeopf":
...
def arg_parse():
    ...
    parser_dataset_params.add_argument("--datatype", help="the type of classification (binary or multiclass) for the powergrid datasets (uk, ieee24, ieee39, ieee118) or regression graph or node power flow", type=str, default="binary")
    ...
    parser_model_params.add_argument(
        "--node_pf_regression",
        help="power flow regression",
        type=str,
        default="False",
    )
    parser_model_params.add_argument(
        "--node_opf_regression",
        help="optimal power flow regression",
        type=str,
        default="False",
    )
